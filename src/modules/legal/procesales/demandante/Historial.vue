...
export default {
  name: 'historial',
  created() {
    this.cargarHistorialDemandante();
  },
  data() {
    return {
      historiales: [],
  ...
  methods: {
    async cargarHistorialDemandante() {
      try {
        const response = await this.$axios.get(`${this.$store.getters.get__url}/demandante/historial/${this.doc}`, {
          headers: {
